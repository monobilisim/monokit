...
// CheckPmgServices checks if all required PMG services are running and updates their status
...
// PostgreSQLStatus checks if PostgreSQL database is running and updates its status
...
// QueuedMessages checks the number of messages in the mail queue and compares it with the configured limit
func QueuedMessages() {

...
func RedisCheck() {
	...
	if err != nil {
		...
		addToVersionErrors(fmt.Errorf("Error getting Redis version: %s", err.Error()))
		return
	}
	...
	if version == "" {
		...
		addToVersionErrors(fmt.Errorf("Unable to parse Redis version"))
		return
	}
	...
	if oldVersion != "" && oldVersion == version {
		...
		addToNotUpdated(AppVersion{Name: "Redis", OldVersion: oldVersion, NewVersion: version})
		...
		addToUpdated(AppVersion{Name: "Redis", OldVersion: oldVersion, NewVersion: version})
		CreateNews("Redis", oldVersion, version, false)
	} else {
		log.Debug().Str("version", version).Msg("Storing initial Redis version")
		addToNotUpdated(AppVersion{Name: "Redis", OldVersion: version})
	}

...
func DockerCheck() {
	...
	if clientVersion != "" {
		...
		if oldClient != "" && oldClient == clientVersion {
			...
			addToNotUpdated(AppVersion{Name: "Docker Client", OldVersion: oldClient, NewVersion: clientVersion})
		...
		addToVersionErrors(fmt.Errorf("Docker client version not detected in output"))
	}
	...
	if serverVersion != "" {
		...
		if oldServer != "" && oldServer == serverVersion {
			...
			addToNotUpdated(AppVersion{Name: "Docker Engine", OldVersion: oldServer, NewVersion: serverVersion})
			...
			addToUpdated(AppVersion{Name: "Docker Engine", OldVersion: oldServer, NewVersion: serverVersion})
			...
			addToNotUpdated(AppVersion{Name: "Docker Engine", OldVersion: serverVersion})
		}
		...
		addToVersionErrors(fmt.Errorf("Docker engine version not detected (daemon may be unavailable)"))
	}
}

...
func Main(cmd *cobra.Command, args []string) {
	...
	for _, unit := range UpCheckConfig.Services {
		...
		if unit == "postgresql.service" {
			unit = resolvePgsqlUnit()
			if unit == "" {
				outputBuilder.WriteString("- postgresql: MISSING\n")
				alarmKey := "upcheck_role_postgresql"
				subject := common.Config.Identifier + " için postgresql servisi bulunamadı"
				body := "Hedef sunucu: " + common.Config.Identifier + "\nServis: postgresql\nDurum: SERVİS BULUNAMADI\n"
				common.AlarmCheckDown(alarmKey, "postgresql service is not installed", false, "", "")
				issues.CheckDown("upcheck/role_postgresql", subject, body, false, 0)
				continue
			}
		}

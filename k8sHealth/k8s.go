...
func podAlarmCheckDown(podName string, namespace string) {
    ...
    // Get deployment
    deployment := GetDeployment(podName)

    if deployment == "" {
        WriteDeployment(podName, namespace)
        deployment = GetDeployment(podName)
    }

    // Check if there is a new pod generated by the deployment
    ...
    if podStillExists == false {
       common.AlarmCheckUp(podName + "_running", "Pod '" + podName + "' from namespace '" + namespace + "' doesn't exist anymore, likely replaced by the Deployment '" + deployment + "'", false)
    } else {
...
func WriteDeployment(name string, namespace string) {
    ...
    for _, deploy := range deployment.Items {
        os.WriteFile(file.Name(), []byte(deploy.Name), 0644)
    }
}
